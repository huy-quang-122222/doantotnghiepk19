...
class Location(models.Model):
    ...
    GrowingRegion = models.ForeignKey(GrowingRegion, on_delete=models.CASCADE,verbose_name='Vùng trồng')  # Liên kết với vùng trồng
    FruitType = [
            ('ngan-ngay', 'ngan-ngay'),
...
class Fruit(models.Model):
    ...
    Location = models.ForeignKey(Location,related_name='fruits', on_delete=models.CASCADE,verbose_name='Địa điểm trồng')  # Trường ngoại khóa đến Location
    def clean(self):
...
class Feedback(models.Model):
    ...
    ParentFeedback = models.ForeignKey('self', on_delete=models.CASCADE, null=True, blank=True, related_name='responses', verbose_name='Bình luận cha')

    # is_notification = models.BooleanField(default=False) 
    notification_message = models.TextField(blank=True, null=True,verbose_name='Nội dung thông báo')
    is_read = models.BooleanField(default=False,verbose_name='Trạng thái thông báo')  # Trường mới để theo dõi trạng thái đã đọc hay chưa
    def __str__(self):
        return self.Content
...
class ProductPrice(models.Model):
    ...
    # def clean(self):
    #     if self.tphcm_price or self.hanoi_price or self.danang_price <= 0:
    #         raise ValidationError("Vui lòng điền giá trị dương")

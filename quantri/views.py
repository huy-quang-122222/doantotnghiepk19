...
from .forms import SignUpForm, FeedbackForm, AvatarUploadForm
import requests
...
# Cchuyển đổi kiểu Decimal(số thập phân) thành kiểu str
...
def display_notifications(request):
    ...
    return []  # Trả về một danh sách trống nếu người dùng chưa đăng nhập hoặc không có thông báo nào.
...
def home(request):
    ...
    seasonPlan = SeasonPlan.objects.all()    #mùa vụ
    ...
    locations_json = [
        ...
        for fruit in location.fruits.all()  # Sử dụng all() để lấy tất cả các đối tượng Fruit liên quan
...
def notification(request, id):
    ...
    return render(request, 'quantri/notification.html',{'display_notification': display_notification, 'count': count, 'notification': notification,})
...
def article_detail(request, id):
    ...
    all_locations = growing_region.location_set.all()  # Lấy tất cả các đối tượng Location của GrowingRegion
    all_locations_info = []
...
def search_view(request):
    ...
    if query:
        results = Article.objects.filter((Q(Title__icontains=query) | Q(Content__icontains=query)| 
                                        Q(FruitID__FruitName__icontains=query)) & Q(IsApproved=True)).order_by('-ArticleID') #icontains tìm kiêm skhoong phân biệt hoa thường
    growing_regions = get_growing_regions()
        ...
        if 'otp' in request.POST:
            ...
            if (timezone.now() - otp_created_at) < timedelta(minutes=10):
                ...
                if user_otp == stored_otp:
                    ...
                    user.email = request.POST['email']  # Lấy giá trị từ ô nhập Email
                    user.save()
                    login(request, user)
...
def weather(request):
    context = {}  # Khởi tạo biến context trước khi gán giá trị
    city_mapping = {
    ...
    if request.method == 'POST':
        selected_city_name = request.POST.get('city')  # Lấy tên thành phố từ form
        ...
        selected_city_name = "Hà Nội"  # Nếu không có yêu cầu POST, đặt giá trị mặc định thành "HÀ NỘI"

    api_key = 'd8d9a74e034f37182ba7e306cc9df2bf'
    ...
    api_url = 'https://provinces.open-api.vn/api/'  # URL của API danh sách tỉnh thành
...
def create_article(request):
    ...
    if request.method == 'POST':
        ...
        user_id = request.user.id  # Lấy user_id từ người dùng hiện tại
        ...
        if not error_message:
            try:
                ...
                location = Location(Latitude=latitude, Longitude=longitude, Name=location_name,FruitType=fruitType, GrowingRegion=selected_region)
                location.save()

                # Tạo bản ghi cho Fruit
                fruit = Fruit(FruitName=fruit_name, Description=fruit_description,  PlantingTime=fruit_planting_time, HarvestTime=fruit_harvest_time, Quantity=quantity, Location=location)
                fruit.save()
                ...
                return redirect('post:home')  # Chuyển hướng đến trang danh sách bài viết sau khi đăng
            except ValidationError as e:
                error_message = "Có lỗi xảy ra: " + str(e)
    
    return render(request, 'quantri/create_article.html', {'error_message': error_message, 'growing_regions': growing_regions})
...
def article_notifications(request, article_id,feedback_id):
    try:
        ...
        all_locations_info = []
        for location in all_locations:
            fruits_info = location.fruits.all()  # Sử dụng all() để lấy tất cả các đối tượng Fruit liên quan
            for fruit in fruits_info:
                fruit_info = {
                    'fruit_name': fruit.FruitName,
                    'location_name': location.Name,
...
def create_growing_region(request):
    if request.method == 'POST':
        ...
        growing_region = GrowingRegion(RegionName=RegionName, Description=Description, Area=Area, MainProduct=MainProduct)
        growing_region.save()
